...
HF_LOCAL_DIR="/state/partition1/user/zzhang1/cache/huggingface"
mkdir -p $HF_LOCAL_DIR
# HF folder in shared file system
HF_USER_DIR="/home/gridsan/zzhang1/.cache/huggingface"
export HF_HOME="${HF_LOCAL_DIR}"

HF_MODEL_NAME="facebook/opt-125m"  # default "opt-125m"
...
#done
 ...
 if [[ $(hostname) =~ login-[1-9] ]]; then
     python load_eval_datasets.py
     python load_eval_models.py --model ${HF_MODEL_NAME}
     python load_baseline_models.py
     rsync -a --ignore-existing ${HF_LOCAL_DIR}/ $HF_USER_DIR
 else
